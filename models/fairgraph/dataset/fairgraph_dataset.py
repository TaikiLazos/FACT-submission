...
class PubMed():
    ...
    def __init__(self, root='nba/'):
        self.name = "PubMed"
        self.dataset = 'pubmed'
        self.data = Planetoid('dataset', 'pubmed')[0]
        self.seed = 20
        self.batch = True
        self.process()

    def process(self):
        edges = np.array([self.data.edge_index[0].numpy(), self.data.edge_index[1].numpy()], dtype=np.int32).T
        self.labels = torch.LongTensor(self.data.y)

        adj = sp.coo_matrix((np.ones(edges.shape[0]), (edges[:, 0], edges[:, 1])),
                            shape=(self.labels.shape[0], self.labels.shape[0]),
                            dtype=np.float32)
        adj = adj + adj.T.multiply(adj.T > adj) - adj.multiply(adj.T > adj)

        train_mask = [index for index, value in enumerate(self.data.train_mask) if value]
        test_mask = [index for index, value in enumerate(self.data.test_mask) if value]
        val_mask = [index for index, value in enumerate(self.data.val_mask) if value]

        self.adj = adj + sp.eye(adj.shape[0])
        self.features = torch.FloatTensor(self.data.x)
        self.features = self.features.cuda()
        self.sens = torch.FloatTensor(self.data.y.float()).cuda()
        self.idx_sens_train = torch.LongTensor(np.where(self.labels>=0)[0]).cuda()
        self.idx_train = torch.LongTensor(train_mask).cuda()
        self.idx_val = torch.LongTensor(val_mask).cuda()
        self.idx_test = torch.LongTensor(test_mask).cuda()
...
class Cora():
    ...
    def __init__(self):
        self.name = "Cora"
        self.dataset = 'cora'
        self.data = Planetoid('dataset', 'cora')[0]
        self.seed = 20
        self.batch = False
        self.process()
...
class Citeseer():
    ...
    def __init__(self, root='nba/'):
        ...
        self.data = Planetoid('dataset', 'citeseer')[0]
        self.seed = 20
        self.batch = False
        self.process()
