...
class aug_module(torch.nn.Module):
    def __init__(self, features, n_hidden=64, temperature=1, state = 'normal') -> None:
    ...
    def forward(self, adj, x, alpha = 0.5, adj_orig = None):
        ...
        # Edge perturbation, to get Graphair w/o EP from Table 2 just set adj_logits to h
        ...
        adj_sampled = adj if self.state == "ep" else adj_sampled

        # Node feature masking, to get Graphair w/o FM from Table 2 just set x_new to x
        ...
        x_new = x * mask
        x_new = x if self.state == "fm" else x_new
